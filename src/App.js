...
const App = () => {
  ...
  const [matchedImage, setMatchedImage] = useState(
    imagesList[0].imageUrl, // Initial matched image
  )
  ...
  const handleThumbnailClick = clickedImage => {
    if (clickedImage === matchedImage) {
      setScore(prevScore => prevScore + 1)
      const randomIndex = Math.floor(Math.random() * imagesList.length)
      setMatchedImage(imagesList[randomIndex].imageUrl)
    } else {
      setTime(0) 
    }
  }
  ...
  return (
    <div className="app-container">
      ...
      <img src={matchedImage} alt="match" className="matched-image" />
    </div>
  )
}
